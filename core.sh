          ...
          if [[ "$state" == "start" ]] && [[ "$PARSED" == "--$BOUNDARY"* ]]; then
            ...
            if [[ "$reader" == "flushing-null" ]]; then
              ...
              if [[ "$PARSED" == "--$BOUNDARY"* ]]; then
                ...
                # Check if this is a file upload or a regular form field
                if [[ -n "$FILENAME" ]]; then
                  ...
                  # This is a regular form field, read its value
                  FORM_DATA["$NAME"]=$(cat "$UPLOAD_TO")
                  rm -f "$UPLOAD_TO"  # Clean up the temp file for form fields
                fi
                MULTIPART_HEADERS=()
                DISPOSITIONS=()
                state="headers"
